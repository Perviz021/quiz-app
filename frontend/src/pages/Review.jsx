...
const getOptionStyle = (optText, optImage, question) => {
  ...
  const optIndex = opts.findIndex(
    (o) => o.text === optText && o.image === optImage,
  );
...
const Review = () => {
  ...
  const wrong = questions.filter(
    (q) => q.selected_option !== -1 && q.selected_option !== q.correct_option,
  ).length;
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div
                key={label}
                className={`${bg} border border-white/10 rounded-xl px-4 py-3`}
              >
                <p className={`text-2xl font-black montserrat-900 ${color}`}>
                  {value}
                </p>
                <p className="text-slate-300 text-[11px] inter mt-0.5">
                  {label}
                </p>
              </div>
            ))}
        ...
        {showSummary && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="px-5 py-3 bg-slate-50 border-b border-slate-100 flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-slate-700 montserrat-700">
                Xülasə Cədvəli
              </h2>
            </div>

            <div className="p-4 overflow-x-auto">
              {Array.from({ length: Math.ceil(summary.length / 10) }).map(
                (_, blockIdx) => {
                  const block = summary.slice(
                    blockIdx * 10,
                    blockIdx * 10 + 10,
                  );
                  return (
                    ...
                    >
                      <tbody>
                        {/* Question numbers */}
                        <tr>
                          <td className="pr-3 py-1.5 text-slate-400 inter font-medium w-28 whitespace-nowrap">
                            №
                          </td>
                          {block.map((item) => (
                            <td
                              key={`num-${item.questionNumber}`}
                              className="text-center px-1 py-1.5 font-bold text-navy montserrat-700"
                            >
                              {item.questionNumber}
                            </td>
                          ))}
                        ...
                        <tr className="border-t border-slate-100">
                          <td className="pr-3 py-1.5 text-slate-400 inter font-medium whitespace-nowrap">
                            Düzgün cavab
                          </td>
                          {block.map((item) => (
                            <td
                        ...
                        <tr className="border-t border-slate-100">
                          ...
                          {block.map((item) => (
                            ...
                            >
                              <span
                                className={`inline-flex items-center justify-center w-6 h-6 rounded-full font-bold montserrat-700 text-[11px] ${
                                  item.studentAnswer === "–"
            ...
            {questions.map((q, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`px-5 py-2.5 flex items-center justify-between border-b ${headerBg}`}
                  >
                    <span
                      className={`text-xs font-bold montserrat-700 uppercase tracking-wider ${headerText}`}
                    >
                      Sual {index + 1}
                    </span>
                    <span
                      className={`inline-flex items-center gap-1 text-xs font-semibold inter ${headerText}`}
                    >
                      {statusIcon}
                      {statusLabel}
                    </span>
                      ...
                      {options.map((opt, optIdx) => {
                        ...
                        return (
                          ...
                          >
                            {/* Letter badge */}
                            <span
                              className={`flex-shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold montserrat-700 mt-0.5 ${
                                style.badge
                            ...
                            <span
                              className={`flex-1 text-sm inter leading-relaxed ${style.text}`}
                            >
                              <ContentBlock
                                text={opt.text}
                                imagePath={opt.image}
                            ...
                            {style.label && (
                              <span
                                className={`flex-shrink-0 text-[10px] font-bold montserrat-700 px-2 py-0.5 rounded-full self-start mt-1 ${
