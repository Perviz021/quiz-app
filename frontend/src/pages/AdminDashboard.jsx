  ...
  <div className="bg-white rounded-2xl shadow-sm border border-border overflow-hidden mb-6">
    <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-slate-50/60">
      <div className="flex items-center gap-3">
        ...
        <h3 className="text-sm font-bold text-gray-800 montserrat-700 tracking-tight">{title}</h3>
      </div>
      {badge}
    </div>
...
const TimeDisplay = ({ seconds }) => {
  ...
  return (
    <span className={`font-mono font-bold text-sm ${seconds < 600 ? "text-red-500" : "text-gray-700"}`}>
      {h}:{m}:{s}
    </span>
  );
};
...
const TH = ({ children, className = "" }) => (
  <th className={`px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-white montserrat ${className}`}>
    {children}
  </th>
);
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/active-students`, {
      ...
      .then((r) => r.json())
      .then((d) => { if (d.error) throw new Error(d.error); setActiveStudents(d.students || []); })
      .catch((e) => toast.error(`Tələbələri yükləmək mümkün olmadı: ${e.message}`));

    fetch(`${API_BASE}/pending-exam-requests`, {
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
    })
      .then((r) => r.json())
      .then((d) => { if (d.error) throw new Error(d.error); setExamRequests(d.requests || []); })
      .catch((e) => toast.error(`İmtahan sorğularını yükləmək mümkün olmadı: ${e.message}`));

    socket.on("connect", () => console.log("🟢 Admin socket connected:", socket.id));
    socket.on("update_active_students", (students) => { console.log("Active students update:", students); setActiveStudents(students || []); });
    socket.on("new_exam_request", (request) => { setExamRequests((prev) => [...prev, request]); toast.info("Yeni imtahan sorğusu gəldi!"); });
    socket.on("student_disconnected", ({ studentId }) => { console.log(`Student ${studentId} disconnected`); setActiveStudents((prev) => prev.filter((s) => s.id !== studentId)); });
    socket.on("error", (message) => { console.error("Socket error:", message); toast.error(`Bağlantı xətası: ${message}`); });
  ...
  const handleForceSubmitConfirm = async () => {
    ...
    try {
      const res  = await fetch(`${API_BASE}/force-submit`, {
        ...
        body: JSON.stringify({ studentId: selectedStudent.id, subjectCode: selectedStudent.subjectCode, forceSubmit: true, answers: [] }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      toast.success("İmtahan bitirildi və tələbə 0 bal aldı.");
      setActiveStudents((prev) => prev.filter((s) => s.id !== selectedStudent.id));
  ...
  const handleExtendTime = (studentId) => { setSelectedStudent(studentId); setIsTimeModalOpen(true); };
  ...
  const handleTimeConfirm = (minutes) => {
    ...
    fetch(`${API_BASE}/extend-time`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${localStorage.getItem("token")}` },
      body: JSON.stringify({ studentId: selectedStudent, subjectCode: student.subjectCode, minutes }),
    })
      .then((r) => r.json())
      .then((d) => {
      ...
      .catch((e) => { console.error("Extend time error:", e); toast.error(`Vaxt əlavə etmək mümkün olmadı: ${e.message}`); });
  };
  ...
  const handleExamParamUpdate = async (e) => {
    ...
    try {
      ...
      toast.error(`İmtahan parametrini yeniləmək mümkün olmadı: ${err.message}`);
    }
  };
  ...
  const handleExamRequest = async (requestId, action) => {
    try {
      ...
      socket.emit("exam_request_response", { studentId: data.studentId, subjectId: data.subjectId, status: action === "approve" ? "approved" : "rejected" });
      toast.success(action === "approve" ? "İmtahan sorğusu təsdiqləndi!" : "İmtahan sorğusu rədd edildi!");
    } catch (err) {
      console.error("Handle exam request error:", err);
      toast.error(`İmtahan sorğusunu ${action === "approve" ? "təsdiqləmək" : "rədd etmək"} mümkün olmadı: ${err.message}`);
    }
  };
    ...
    <div className="min-h-screen bg-surface">
      ...
      <div className="bg-navy-mid">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              ...
              {
                value: <span className="text-emerald-300 text-sm font-semibold inter">Aktiv</span>,
              ...
              <div key={i} className="bg-white/10 border border-white/15 rounded-xl px-4 py-3 flex items-center gap-3">
                <div className={`w-9 h-9 rounded-lg ${color} flex items-center justify-center flex-shrink-0`}>
                  {icon}
                </div>
                <div>
                  <p className="text-2xl font-bold text-white montserrat-700 leading-none">{value}</p>
                  <p className="text-blue-200 text-[11px] inter mt-0.5">{label}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <svg viewBox="0 0 1440 24" className="w-full block" preserveAspectRatio="none" style={{ height: "24px" }}>
          <path d="M0,24 C360,0 1080,0 1440,24 L1440,24 L0,24 Z" fill="var(--color-surface, #f4f6fa)" />
        </svg>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <SectionCard
          ...
          icon={
            <svg className="w-4 h-4 text-gold-light" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
            </svg>
          }
          badge={
        ...
        >
          {examRequests.length === 0 ? (
            <EmptyState
              ...
              icon={<svg className="w-6 h-6 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
            />
          ) : (
            <>
                ...
                <table className="w-full border-collapse text-sm">
                  ...
                  <tbody className="divide-y divide-border">
                    {examRequests.map((req, idx) => (
                      <tr key={req.id} className={`transition-colors hover:bg-blue-50/40 ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/40"}`}>
                        <td className="px-4 py-3 font-semibold text-gray-800 inter">{req.studentName}</td>
                        <td className="px-4 py-3 text-gray-600 inter">{req.subjectName}</td>
                        <td className="px-4 py-3 text-gray-500 text-xs inter hidden sm:table-cell">
                          {new Date(req.requestTime).toLocaleString()}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            <button onClick={() => handleExamRequest(req.id, "approve")} className="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer montserrat-600">Təsdiqlə</button>
                            <button onClick={() => handleExamRequest(req.id, "reject")}  className="bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer montserrat-600">Rədd Et</button>
                          </div>
                        </td>
                      </tr>
                    ))}
        ...
        >
          <form onSubmit={handleExamParamUpdate}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider montserrat mb-1.5">Tələbə Kodu</label>
                <input type="text" value={examParamForm.studentId} onChange={(e) => setExamParamForm((p) => ({ ...p, studentId: e.target.value }))} className={inputClass} required placeholder="XXXXXXXX" />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider montserrat mb-1.5">Fənn Qrupu</label>
                <input type="text" value={examParamForm.subjectGroup} onChange={(e) => setExamParamForm((p) => ({ ...p, subjectGroup: e.target.value }))} className={inputClass} required placeholder="Qrup kodu" />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider montserrat mb-1.5">İmtahan Parametri</label>
                <input type="number" value={examParamForm.newEP} onChange={(e) => setExamParamForm((p) => ({ ...p, newEP: e.target.value }))} className={inputClass} required min="0" placeholder="10 / 31 / 32" />
                <p className="text-[11px] text-slate-400 mt-1.5 inter">10 = icazə · 31 = qayıb · 32 = təhsil haqqı</p>
              </div>
            </div>
            <button type="submit" className="flex items-center gap-2 bg-navy hover:bg-navy-light text-white text-xs font-bold px-5 py-2.5 rounded-lg transition-colors cursor-pointer montserrat-700">
              ...
              <table className="w-full border-collapse text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {activeStudents?.map((student, idx) => (
                    <tr key={student.id} className={`transition-colors hover:bg-blue-50/40 ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/40"}`}>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="font-semibold text-gray-800 inter">{student.fullname}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-gray-600 inter hidden sm:table-cell">{student.subject}</td>
                      <td className="px-4 py-3"><TimeDisplay seconds={student.timeLeft} /></td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          <button onClick={() => handleForceSubmitClick(student)} className="bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer montserrat-600 whitespace-nowrap">Bitir</button>
                          <button onClick={() => handleExtendTime(student.id)} className="bg-navy hover:bg-navy-light text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer montserrat-600 whitespace-nowrap">Vaxt +</button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      <TimeExtensionModal isOpen={isTimeModalOpen} onClose={() => setIsTimeModalOpen(false)} onConfirm={handleTimeConfirm} />
      <ForceSubmitModal
        isOpen={isForceSubmitModalOpen}
        onClose={() => { setIsForceSubmitModalOpen(false); setSelectedStudent(null); }}
